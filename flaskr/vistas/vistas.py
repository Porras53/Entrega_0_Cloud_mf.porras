...
evento_schema = modelos.EventoSchema()
usuario_schema = modelos.UsuarioSchema()

class VistaEventos(Resource):

    def post(self):
        nuevo_evento = modelos.Evento(nombre=request.json["nombre"], \
        ...
        modelos.db.session.add(nuevo_evento)
        modelos.db.session.commit()
        return evento_schema.dump(nuevo_evento)

    def get(self):
        return [evento_schema.dump(evento) for evento in modelos.Evento.query.all()]

class VistaEvento(Resource):

    def get(self, id_evento):
        return evento_schema.dump(modelos.Evento.query.get_or_404(id_evento))

    def put(self, id_evento):
        evento = modelos.Evento.query.get_or_404(id_evento)
    ...
    def delete(self, id_evento):
        evento = modelos.Evento.query.get_or_404(id_evento)
        modelos.db.session.delete(evento)
        modelos.db.session.commit()
        return '',204

class VistaLogIn(Resource):
    def post(self):
        usuario = modelos.Usuario.query.filter(modelos.Usuario.nombre == request.json["nombre"], Usuario.contrasena == request.json["contrasena"]).first()
        modelos.db.session.commit()
        if usuario is None:
...
class VistaSignIn(Resource):
    ...
    def post(self):
        nuevo_usuario = modelos.Usuario(nombre=request.json["nombre"], contrasena=request.json["contrasena"])
        token_de_acceso= create_access_token(identity=request.json['nombre'])
        modelos.db.session.add(nuevo_usuario)
        modelos.db.session.commit()
        return {'mensaje':'usuario creado exitosamente','token':token_de_acceso}

    def put(self, id_usuario):
        usuario = modelos.Usuario.query.get_or_404(id_usuario)
    ...
    def delete(self, id_usuario):
        usuario = modelos.Usuario.query.get_or_404(id_usuario)
        modelos.db.session.delete(usuario)
        modelos.db.session.commit()
        return '',204
